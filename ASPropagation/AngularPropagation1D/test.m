...
for t = 1:length(diff_n)
    ...
    Lx = L * cot(theta_r); Ly = 100e-6; % Adjust spans to focus on x>0, z<0
    N = 1000;
    ...
    %origin = [0, 0, -delta_z];
    origin = [delta_x, 0, delta_z]; % Align origin with beam direction
    [u, v] = meshgrid(linspace(0, Lx, N), linspace(-Ly/2, Ly/2, N));
    ...
    s_line = X_line_plane; % X = s*cos(theta) => s = X/cos(theta)
