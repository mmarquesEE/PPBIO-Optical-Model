...
% ============================================================
% --- NEW: Set up the Regularization ---
% ============================================================
% 1. Choose the regularization parameter, lambda.
%    A larger lambda means a smoother result. This often requires tuning.
lambda = 0.5; 

% 2. Create the regularization matrix L
L = create_regularization_matrix(ads_x_dim, ads_y_dim);
% ============================================================

% Create a handle to the plotter function (no change here)
...
optim_opts = optimoptions('lsqnonlin', ...
    'Algorithm', 'trust-region-reflective', 'Display', 'iter', 'MaxIterations', 130, ...
    'UseParallel', true, 'FunctionTolerance', 1e-10, 'StepTolerance', 1e-10, ...
    'MaxFunctionEvaluations',2500, 'OutputFcn', optim_plot_fun);
    
% --- CHANGE: Update the residual function handle ---
% It now points to our new regularized function and passes L and lambda
residual_fun = @(log_params) compute_regularized_residuals_2D(log_params, L, lambda, exp_settings, exp_data, model_config);

% --- Run Optimization ---
tic;
[opt_log_params, resnorm, residual, exitflag, output, lambda_lsq, J_opt] = lsqnonlin(residual_fun, p0, lb, ub, optim_opts);
toc;

% --- Analyze and plot final results ---
% NOTE: The confidence intervals calculated from a regularized problem have a
% different statistical interpretation, as they are now biased towards the
% smoothing prior. They reflect the uncertainty of the *regularized* solution.
p_init = 10.^p0;
