...
safety_margin = 1.1;
n_end_point = n_bulk + (n_eff - n_bulk) * safety_margin;
% Set the number of points to match the camera's horizontal resolution.
num_calibration_points = vidWidth; 

fprintf('Automatically determined calibration range: [%.5f, %.5f] with %d points.\n', n_bulk, n_end_point, num_calibration_points);
n2_calibration_range = linspace(n_bulk, n_end_point(end), num_calibration_points);

% --- Step D: Calculate the corresponding angles (this part is unchanged) ---
...
% --- Clean up the curve for interpolation (this part is unchanged) ---
...
% This line creates the 'ru_reconstructed' variable that was missing.
