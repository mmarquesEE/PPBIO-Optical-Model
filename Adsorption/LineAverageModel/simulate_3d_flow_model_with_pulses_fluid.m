function [t_all, c_s, s] = simulate_3d_flow_model_with_pulses_fluid(...
    ...
    % =========================================================================
    % --- ALTERATION: Reshape output to [nx, ny, nz, time] format ---
    % The visualization script expects the time dimension to be the 4th dimension.
    % The raw output `y_all` has the shape [time, flattened_space].
    
    % First, extract the data for concentration and transpose it so that the
    % shape becomes [flattened_space, time].
    c_s_flat_transposed = y_all(:, 1:num_cells)';
    s_flat_transposed = y_all(:, num_cells+1:2*num_cells)';
    
    % Now, reshape into the desired 4D format: [nx, ny, nz, time]
    c_s = reshape(c_s_flat_transposed, [nx, ny, nz, length(t_all)]);
    s = reshape(s_flat_transposed, [nx, ny, nz, length(t_all)]);
    % =========================================================================
