function heterogeneous_binding_analysis()
    ...
    t_association = 1300;   % Reduced time for testing
    t_dissociation = 1700;
    ...
    % Define adsorption region grid cells
    ads_x_cells = ads_x_range(1):ads_x_range(2);  % 10-15 (6 cells)
    ads_y_cells = ads_y_range(1):ads_y_range(2);   % 2-4 (3 cells)
    num_ads_cells = length(ads_x_cells) * length(ads_y_cells);
    
    % ================ SYNTHETIC DATA GENERATION ================
    fprintf('Generating synthetic data...\n');
    
    % Create random ground truth kon values (log-spaced between 1e-1 and 1e4)
    rng(1);  % For reproducibility
    true_kon = 10.^(rand(num_ads_cells,1)*2 - 1);  % 1e-1 to 1e4
    true_kon_grid = reshape(true_kon, length(ads_x_cells), length(ads_y_cells)); % 6×3
    % Create parameter grids
    [kon_grid, koff_grid, smax_grid] = generate_hetero_param_grids(...
        gridN_x, gridN_y, gridN_z, ads_x_range, ads_y_range, ads_layer, true_kon_grid);
    ...
    p_opt = fmincon(@(p) cost_function(p, s_obs, gridN_x, gridN_y, gridN_z,...
        ...
        c0_diss, t_association, t_dissociation, D_coeff, ru_to_m, lambda),...
        p0, [], [], [], [], lb, ub, [], options);
    % Convert back to linear space
    recovered_kon = 10.^p_opt;
...
function [kon_grid, koff_grid, smax_grid] = generate_hetero_param_grids(...
    nx, ny, nz, ads_x_range, ads_y_range, ads_layer, kon_values)
    
    kon_grid = zeros(nx, ny, nz);
    koff_grid = zeros(nx, ny, nz);  % Homogeneous koff for this example
    smax_grid = zeros(nx, ny, nz);
    
    x_idx = ads_x_range(1):ads_x_range(2);
    y_idx = ads_y_range(1):ads_y_range(2);
    
    % Assign cell-specific kon values
    kon_grid(x_idx, y_idx, ads_layer) = kon_values;
    
    % Set constant koff and smax
    koff_grid(x_idx, y_idx, ads_layer) = 1e-2;  % Fixed koff
    smax_grid(x_idx, y_idx, ads_layer) = 1;      % Fixed smax
end
...
function [cost, s_fit] = cost_function(p, s_obs, nx, ny, nz, ads_x_range,...
    ...
    t_assoc, t_total, D_coeff, ru_to_m, lambda)
    ...
    [kon_3d, koff_3d, smax_3d] = generate_hetero_param_grids(...
        nx, ny, nz, ads_x_range, ads_y_range, ads_layer, kon_grid);
