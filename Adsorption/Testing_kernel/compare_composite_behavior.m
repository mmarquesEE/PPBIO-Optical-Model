function compare_composite_behavior(s_homog, s_heterog, Q_homog, Q_heterog, K_homog, K_heterog,...
        t_homog, t_heterog, ads_x_range, ads_y_range, ads_layer,...
        kon_homog, smax_per_cell, kon_heterog_ads, smax_heterog_ads)
    
    % --- Define Publication Style Parameters ---
    target_fig_width_cm = 8.4;
    base_font_size = 8;
    line_width = 1.2;
    line_width_thick = 1.5;

    % --- Data processing (code is unchanged) ---
    ads_cells_s_homog = s_homog(:, ads_x_range(1):ads_x_range(2), ads_y_range(1):ads_y_range(2), ads_layer);
    ads_cells_Q_homog = Q_homog(:, ads_x_range(1):ads_x_range(2), ads_y_range(1):ads_y_range(2), ads_layer);
    ads_cells_K_homog = K_homog(:, ads_x_range(1):ads_x_range(2), ads_y_range(1):ads_y_range(2), ads_layer);
    ads_cells_s_heterog = s_heterog(:, ads_x_range(1):ads_x_range(2), ads_y_range(1):ads_y_range(2), ads_layer);
    ads_cells_Q_heterog = Q_heterog(:, ads_x_range(1):ads_x_range(2), ads_y_range(1):ads_y_range(2), ads_layer);
    ads_cells_K_heterog = K_heterog(:, ads_x_range(1):ads_x_range(2), ads_y_range(1):ads_y_range(2), ads_layer);
    s0_homog_ads = squeeze(ads_cells_s_homog(1, :, :));
    s0_heterog_ads = squeeze(ads_cells_s_heterog(1, :, :));
    alpha_homog_ads = kon_homog * smax_per_cell * ones(size(kon_heterog_ads));
    alpha_heterog_ads = kon_heterog_ads .* smax_heterog_ads;
    alpha_homog_ads = reshape(alpha_homog_ads, [1, size(alpha_homog_ads)]);
    alpha_heterog_ads = reshape(alpha_heterog_ads, [1, size(alpha_heterog_ads)]);
    s0_homog_ads = reshape(s0_homog_ads, [1, size(s0_homog_ads)]);
    s0_heterog_ads = reshape(s0_heterog_ads, [1, size(s0_heterog_ads)]);
    decay_term_homog = s0_homog_ads .* exp(-ads_cells_Q_homog);
    decay_term_heterog = s0_heterog_ads .* exp(-ads_cells_Q_heterog);
    kernel_term_homog = alpha_homog_ads .* ads_cells_K_homog;
    kernel_term_heterog = alpha_heterog_ads .* ads_cells_K_heterog;
    decay_sum_homog = squeeze(sum(decay_term_homog, [2,3]));
    kernel_sum_homog = squeeze(sum(kernel_term_homog, [2,3]));
    s_obs_homog = decay_sum_homog + kernel_sum_homog;
    decay_sum_heterog = squeeze(sum(decay_term_heterog, [2,3]));
    kernel_sum_heterog = squeeze(sum(kernel_term_heterog, [2,3]));
    s_obs_heterog = decay_sum_heterog + kernel_sum_heterog;
    discrepancy = trapz(t_heterog, (s_obs_homog - s_obs_heterog).^2);
    fprintf('Composite behavior discrepancy: %.2e\n', discrepancy);

    % =========================================================================
    % --- FIGURE 1: Signal Decomposition ---
    % =========================================================================
    fig1 = figure('Name', 'Signal Decomposition');
    t1 = tiledlayout(2, 1, 'TileSpacing', 'compact', 'Padding', 'compact');
    
    % --- Panel (A) ---
    ax1 = nexttile;
    plot(ax1, t_homog, s_obs_homog, 'k-', 'LineWidth', line_width_thick, 'DisplayName', 'Total'); hold(ax1, 'on');
    plot(ax1, t_homog, decay_sum_homog, 'b--', 'LineWidth', line_width, 'DisplayName', 'Decay Term');
    plot(ax1, t_homog, kernel_sum_homog, 'r--', 'LineWidth', line_width, 'DisplayName', 'Kernel Term');
    hold(ax1, 'off'); grid on; box on;
    ylabel(ax1, 's_{obs}', 'FontSize', base_font_size);
    title(ax1, '(A) Homogeneous Case', 'FontSize', base_font_size);
    set(ax1, 'XTickLabel', [], 'FontSize', base_font_size - 1);
    
    % --- Panel (B) ---
    ax2 = nexttile;
    plot(ax2, t_heterog, s_obs_heterog, 'k-', 'LineWidth', line_width_thick, 'DisplayName', 'Total'); hold on;
    plot(ax2, t_heterog, decay_sum_heterog, 'b--', 'LineWidth', line_width, 'DisplayName', 'Decay Term');
    plot(ax2, t_heterog, kernel_sum_heterog, 'r--', 'LineWidth', line_width, 'DisplayName', 'Kernel Term');
    hold off; grid on; box on;
    ylabel(ax2, 's_{obs}', 'FontSize', base_font_size);
    title(ax2, '(B) Heterogeneous Case', 'FontSize', base_font_size);
    set(ax2, 'FontSize', base_font_size - 1);

    % --- Add shared elements ---
    xlabel(t1, 'Time (s)', 'FontSize', base_font_size);
    lgd = legend(ax1); % Create legend from the first plot
    lgd.Layout.Tile = 'North'; % Place legend in a shared space at the top
    lgd.NumColumns = 3;
    lgd.Box = 'off';
    lgd.FontSize = base_font_size - 1;

    save_pub_fig(fig1, 'Adsorption/Testing_kernel/Figures/signal_decomposition_pub', target_fig_width_cm);
    close(fig1);

    % =========================================================================
    % --- FIGURE 2: Sensorgram Comparison ---
    % =========================================================================
    fig2 = figure('Name', 'Sensorgram Comparison');
    ax = gca;
    plot(ax, t_heterog, s_obs_homog, 'b-', 'LineWidth', line_width, 'DisplayName', 'Homogeneous'); hold(ax, 'on');
    plot(ax, t_heterog, s_obs_heterog, 'r--', 'LineWidth', line_width, 'DisplayName', 'Heterogeneous (5% var)');
    hold(ax, 'off'); grid on; box on;
    xlabel(ax, 'Time (s)', 'FontSize', base_font_size);
    ylabel(ax, 's_{obs}(t)', 'FontSize', base_font_size);
    legend(ax, 'Location', 'best', 'FontSize', base_font_size - 1);
    %title(ax, 'Composite Behavior Comparison', 'FontSize', base_font_size);
    set(ax, 'FontSize', base_font_size - 1);
    
    save_pub_fig(fig2, 'Adsorption/Testing_kernel/Figures/sensorgram_comparison_pub', target_fig_width_cm);
    close(fig2);
end