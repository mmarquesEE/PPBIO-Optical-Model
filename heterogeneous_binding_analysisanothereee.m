...
function [K_homog, t_homog] = precompute_homogeneous_kernel(homog_params, gridN_x, gridN_y, gridN_z, ads_x_range, ads_y_range, ads_layer, velocity_profile, c0_assoc, c0_diss, t_assoc, t_total, D_coeff, ru_to_m)
    % Create homogeneous parameter grids
    kon = homog_params(1);
    koff = homog_params(2);
    smax_total = homog_params(3);
    num_ads_cells = (ads_x_range(2)-ads_x_range(1)+1) * (ads_y_range(2)-ads_y_range(1)+1);
    smax_per_cell = smax_total / num_ads_cells;
...
function [cost, s_fit] = theorem_constrained_cost(p, s_obs, homog_params, gridN_x, gridN_y, gridN_z, ads_x_range, ads_y_range, ads_layer, velocity_profile, c0_assoc, c0_diss, t_assoc, t_total, D_coeff, ru_to_m, lambda_kon, lambda_koff, lambda_smax, lambda_mean)
    ...
    % Create parameter grids (unchanged)
    kon_grid = reshape(kon_vals, [length(ads_x_range(1):ads_x_range(2)), length(ads_y_range(1):ads_y_range(2))]);
    koff_grid = reshape(koff_vals, size(kon_grid));
    smax_grid = reshape(smax_vals, size(kon_grid));
    
    [kon_3d, koff_3d, smax_3d] = generate_hetero_param_grids(gridN_x, gridN_y, gridN_z, ads_x_range, ads_y_range, ads_layer, kon_grid, koff_grid, smax_grid);
    
    % Run simulation and get K_heterog
    [t_heterog, ~, s, K_heterog] = simulate_3d_flow_model(gridN_x, gridN_y, gridN_z, kon_3d, koff_3d, smax_3d, velocity_profile, c0_assoc, c0_diss, t_assoc, t_total, D_coeff, ru_to_m);
    
    % Precompute homogeneous kernel (once)
    if isempty(K_homog)
        [K_homog, ~] = precompute_homogeneous_kernel(homog_params, gridN_x, gridN_y, gridN_z, ads_x_range, ads_y_range, ads_layer, velocity_profile, c0_assoc, c0_diss, t_assoc, t_total, D_coeff, ru_to_m);
    end
    
    % Extract adsorption region cells
    ads_x_cells = ads_x_range(1):ads_x_range(2);
    ads_y_cells = ads_y_range(1):ads_y_range(2);
    ads_cells = K_heterog(:, ads_x_cells, ads_y_cells, ads_layer);
    
    % Compute αK for heterogeneous case
    alpha_heterog = kon_vals .* smax_vals;
    alpha_heterog = reshape(alpha_heterog, [1, length(ads_x_cells), length(ads_y_cells), 1]);
    sum_alphaK_heterog = squeeze(sum(alpha_heterog .* ads_cells, [2, 3, 4]));
    
    % Compute αK for homogeneous case
    ...
    % Total cost
    cost =misfit + lambda_mean * kernel_misfit + reg_kon + reg_koff + reg_smax;
end
