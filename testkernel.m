...
function validate_kernel_implementation()
    ...
    [K_homog, t_homog] = simulate_homogeneous_kernel(homog_params, gridN_x, gridN_y, gridN_z, ads_x_range, ads_y_range, ads_layer);
    ...
    [K_heterog, t_heterog] = simulate_heterogeneous_kernel(kon_grid, koff_grid, smax_grid, gridN_x, gridN_y, gridN_z);
    
    % 3. Composite Behavior Comparison
    compare_composite_behavior(K_homog, K_heterog, t_homog, t_heterog,...
        kon_grid(ads_x_range(1):ads_x_range(2), ads_y_range(1):ads_y_range(2), ads_layer),...
        koff_grid(ads_x_range(1):ads_x_range(2), ads_y_range(1):ads_y_range(2), ads_layer),...
        smax_grid(ads_x_range(1):ads_x_range(2), ads_y_range(1):ads_y_range(2), ads_layer),...
        homog_params, num_ads_cells,ads_x_range, ads_y_range, ads_layer);
end
...
function [K_homog, t_homog] = simulate_homogeneous_kernel(params, nx, ny, nz, ads_x_range, ads_y_range, ads_layer)
    ...
    [t_homog, ~, ~, K_homog] = simulate_3d_flow_model(...
        nx, ny, nz, kon_grid, koff_grid, smax_grid,...
        velocity_profile, 3.3e-6, 0, 800, 1200, 6e-3, 1e-6);
end
...
function [kon_grid, koff_grid, smax_grid] = create_ground_truth_heterogeneity(nx, ny, nz, ads_x_range, ads_y_range, ads_layer)
    ...
    kon_vals = kon_base * (1 + 0.5*randn(ads_x_range(2)-ads_x_range(1)+1, ads_y_range(2)-ads_y_range(1)+1));
    koff_vals = koff_base * (1 + 0.5*randn(size(kon_vals)));
    smax_vals = (smax_total/num_ads_cells) * (1 + 0.5*randn(size(kon_vals)));
...
function [K_heterog, t_heterog] = simulate_heterogeneous_kernel(kon_grid, koff_grid, smax_grid, nx, ny, nz)
    velocity_profile = create_velocity_profile(nz, 8.3);
    [t_heterog, ~, ~, K_heterog] = simulate_3d_flow_model(...
        nx, ny, nz, kon_grid, koff_grid, smax_grid,...
        velocity_profile, 3.3e-6, 0, 800, 1200, 6e-3, 1e-6);
end

function compare_composite_behavior(K_homog, K_heterog, t_homog, t_heterog, kon_heterog, koff_heterog, smax_heterog, homog_params, num_ads_cells,ads_x_range, ads_y_range, ads_layer)
    % Homogeneous composite term
    alpha_homog = homog_params(1) * homog_params(3);
    sum_alphaK_homog = alpha_homog * squeeze(mean(K_homog(:, ads_x_range(1):ads_x_range(2), ads_y_range(1):ads_y_range(2), ads_layer), [2,3,4])); % Sum over all cells
    
    % Heterogeneous composite term
    alpha_heterog = kon_heterog .* smax_heterog;
    
    % Extract adsorption region from K_heterog
    ads_cells_K = K_heterog(:, ads_x_range(1):ads_x_range(2), ads_y_range(1):ads_y_range(2), ads_layer, :);
    
    % Reshape alpha_heterog to [1, nx_ads, ny_ads, 1] for broadcasting
    alpha_heterog_reshaped = reshape(alpha_heterog, [1, size(alpha_heterog)]);
    
    % Calculate composite sum
    sum_alphaK_heterog = squeeze(sum(alpha_heterog_reshaped .* ads_cells_K, [2,3,4]));
        
    % Calculate discrepancy
    discrepancy = trapz(t_heterog, (sum_alphaK_homog - sum_alphaK_heterog).^2);
    fprintf('Composite kernel discrepancy: %.2e\n', discrepancy);
    
    % Plot results
    figure;
    plot(t_heterog, sum_alphaK_homog, 'b-', 'LineWidth', 2); hold on;
    plot(t_heterog, sum_alphaK_heterog, 'r--', 'LineWidth', 1.5);
    xlabel('Time (s)'); ylabel('\Sigma(\alpha K)');
    legend('Homogeneous', 'Heterogeneous (5% var)');
    title(sprintf('Composite Kernel Comparison\nDiscrepancy: %.2e', discrepancy));
    grid on;
end
