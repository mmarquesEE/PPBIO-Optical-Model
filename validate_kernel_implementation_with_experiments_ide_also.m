function validate_kernel_implementation_with_experiments()
    ...
    ads_layer = 1;
    ads_x_range = [5,5]; ads_y_range = [3,3];
    ...
    true_params = [true_kon(:); true_koff(:); true_smax(:)];
    ...
    ub = inf(size(true_params));
    
    % Initial guess (20% perturbation from true values)
    rng(42); % For reproducibility
    initial_guess = true_params .* (0.8 + 0.4*rand(size(true_params)));
    
    % Run optimization
    [opt_params, fval, exitflag] = fmincon(...
        @(p) objective_function(p, exp_settings, exp_data, gridN_x, gridN_y, gridN_z, ...
        ads_x_range, ads_y_range, ads_layer, D_coeff, ru_to_m), ...
        initial_guess, [], [], [], [], lb, ub, [], optim_opts);
    
    % Analyze results
    fprintf('\nOptimization Results:\n');
    fprintf('Final Objective Value: %.4e\n', fval);
    fprintf('Exit Flag: %d\n', exitflag);
    
    % Calculate parameter errors
    param_errors = abs(opt_params - true_params) ./ true_params;
    fprintf('\nParameter Recovery Accuracy:\n');
    fprintf('Mean Relative Error: %.2f%%\n', 100*mean(param_errors));
    fprintf('Max Relative Error: %.2f%%\n', 100*max(param_errors));
    
    % Plot parameter recovery
    plot_parameter_recovery(true_params, opt_params, initial_guess, ...
        size(true_kon), size(true_koff), size(true_smax));
    
    % Plot predicted vs "experimental" signals
    plot_signal_predictions(opt_params, exp_settings, exp_data, gridN_x, gridN_y, gridN_z, ...
        ads_x_range, ads_y_range, ads_layer, D_coeff, ru_to_m);
end
...
%% New helper function for creating grids from adsorption parameters
